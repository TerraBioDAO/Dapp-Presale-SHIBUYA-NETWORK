...
export const MetamaskContextProvider = ({ children }) => {
  ...
  const checkApprove = async (nb, owner, spender, value) => {
    if(nb === 0) {
      const cb = (paymentReceiver, amount, proposalId) => {
        ...
        usdcContract.on("Approval", cb)
          return () => {
            usdcContract.off("Approval", cb)
        };
      ...
      const cb = (paymentReceiver, amount, proposalId) => {
        ...
        usdtContract.on("Approval", cb)
          return () => {
            usdtContract.off("Approval", cb)
        };
    }
  };
  ...
  const checkTransfer = async (sender, amount) => {
    ...
    const cb = (sender, amount) => {
      ...
      presaleContract.on("Deposited", cb)
        return () => {
          presaleContract.off("Deposited", cb)
      };
    };
